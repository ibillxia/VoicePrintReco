...
# This script process the acoustic feature files extracted from the SPHERE files by using HTK
# If those features have not been generated (in directory ./data/prm/) you need to run 
#
#	01_RUN_feature_extraction.sh
#
# with option: FEATURE_TYPE="HTK"
#
...
# as well as label files indicating the time label of speech frames in ./data/lbl/
	...
	CMD_NORM_E="bin/NormFeat --config cfg/NormFeat_energy_HTK.cfg --inputFeatureFilename data/data.lst --featureFilesPath data/prm/"
	echo $CMD_NORM_E
	$CMD_NORM_E
        echo "End normalise energy : `date`\n "

        echo "Energy Detector : `date` "
        CMD_ENERGY="bin/EnergyDetector  --config cfg/EnergyDetector_HTK.cfg --inputFeatureFilename data/data.lst --featureFilesPath data/prm/  --labelFilesPath  data/lbl/"
	echo $CMD_ENERGY
	$CMD_ENERGY
        echo "End energy detector : `date`\n "

        echo "Normalise Features : `date`"
        CMD_NORM="bin/NormFeat --config cfg/NormFeat_HTK.cfg --inputFeatureFilename data/data.lst --featureFilesPath  data/prm/   --labelFilesPath  data/lbl/"
	echo $CMD_NORM
	$CMD_NORM
